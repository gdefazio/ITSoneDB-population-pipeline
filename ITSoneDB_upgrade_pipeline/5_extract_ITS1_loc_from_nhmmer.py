...
def main(path_folder_of_folders, path_fasta, outname, processes, release):
    ...
    if os.path.exists(path_folder_of_folders):
        ...
        try:
            ...
            os.mkdir(tmp_path)
        # print('+++', tmp_path)
        list_folders = os.listdir(path_folder_of_folders)
            # [folder for folder in os.listdir(path_folder_of_folders)
        ...
        with mp.Pool(processes=processes, maxtasksperchild=1) as po:
            args = [(folder, path_folder_of_folders, path_fasta, tmp_path) for
                    folder in list_folders if folder.endswith('tmp') is False]
