...
def inner_splitter(open_file):
    ...
    del stringa
    for el in s:
        acc = el.split(';')[0].strip()
        data = "ID%s" % el.replace('//','')
        acc2data[acc] = data
    del s
    return acc2data
...
def define_section_line(data_list):
    section2line = {}
    excl_line = []
    for section in ["ID", "AC", "AH", "AS", "DE", "KW", "DT", "OS", "OC", "RT", "RP",
                    "RN", "RF", "RX", "RA", "RL", "DT", "DR", "FT", "FH", "SQ"]:
        # print section
        i = 0
...
def table_parser(item_list):
    ...
    if len( item_list ) != 2:
        ...
        # print(section)
        if 'OS' in section:
            for line in data_file[section["OS"]:section["OS"] + 5]:
                s = list(map( str.strip, line.split() ))
                if s[0] == "OS":
                    species = " ".join( s[1:] )
                elif s[0] == "OC":
...
def file_splitter(file_path):
    ...
    try:
        ...
        if os.path.exists(file_path) is False:
            print( "the indicated flat file (%s) doesn't exists" % name)
            return 0
        # elif os.path.exists(os.path.join(base, "tsv_%s" % release, "%s.tsv.gz" % name)) and \
        #     os.path.exists(os.path.join(base, "fasta_%s" % release, "%s.fasta.gz" % name)):
        #     print("this file (%s) has been already processed" % name)
        #     return 1
        else:
...
if __name__ == '__main__':
    ...
    param = split_options()
    processes, release, input_dir, base = param.process, param.release, param.input, param.output
    ...
    try:
        ...
        pass
    # acc2data = file_splitter(os.path.join(input_dir, "STD_INV_31.dat.gz"))
    # else:
    ...
    with mp.Pool(processes=processes, initargs=[release], maxtasksperchild=1) as pool:
        check_list = pool.map(func=file_splitter,
                              iterable=list_file,
                              chunksize=1)
    pool.join()
    pool.close()
