...
#    echo ${gauxdp}/solo_fungi_5_8S.hmm
    conda run -n itsonedb nhmmer -o ${out_dir}/${folder}/align_5_8S_data.txt --cpu ${thread} -T 5 ${gauxdp}/solo_fungi_5_8S.hmm ${inputdir}/${compressed_fasta}
    conda run -n itsonedb nhmmer -o ${out_dir}/${folder}/align_18S_data.txt  --cpu ${thread} -T 5 ${gauxdp}/total_rfam_18S.hmm ${inputdir}/${compressed_fasta}
    #cd $main_folder
